...
static uint16 simiic_delay_time = 100;   //ICM等传感器应设置为2
//simiic_delay_time = 1 在内核频率为104M的时候  CLK频率为700K
//simiic_delay_time = 2 在内核频率为104M的时候  CLK频率为625K
//simiic_delay_time = 3 在内核频率为104M的时候  CLK频率为570K
//-------------------------------------------------------------------------------------------------------------------
//  @brief      模拟IIC延时 时间设置
//  @return     void
//  @since      v1.0
...
{
    vuint16 delay_time = simiic_delay_time;
    while(delay_time--);
}
...
{
    gpio_init (SEEKFREE_SCL, GPO, 1, SPEED_50MHZ | OUT_PP);
    gpio_init (SEEKFREE_SDA, GPO, 1, SPEED_50MHZ | OUT_OD);
}
